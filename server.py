...
def build_state() -> dict[str, Any]:
    ...
    return {
        ...
        'valid_until': run(['bash','-c','openssl s_client -connect unicaizer.ru:443 -servername unicaizer.ru </dev/null 2>/dev/null | openssl x509 -noout -enddate 2>/dev/null | cut -d= -f2'], 10).strip() or '—',
...
<style>
*{{box-sizing:border-box}}body{{margin:0;font-family:Inter,'SF Pro Display',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;background:#f5f7fa;color:#1a1e2c;font-size:12px;line-height:1.35}}a{{color:#2563eb;text-decoration:none}}a:hover{{text-decoration:underline;color:#1d4ed8}}.wrap{{max-width:1500px;margin:0 auto;padding:8px}}header{{display:flex;justify-content:space-between;gap:8px;align-items:flex-start;margin-bottom:6px;background:#fff;border-radius:10px;padding:8px 12px;box-shadow:0 1px 2px #0001;border:1px solid #e2e8f0}}h1{{margin:0;font-size:18px;font-weight:700;letter-spacing:-.03em;color:#0f172a}}.server-meta{{display:flex;flex-wrap:wrap;gap:4px;margin-top:4px}}.server-meta span{{display:inline-flex;align-items:center;gap:3px;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:999px;padding:2px 8px;font-size:10px;color:#475569;white-space:nowrap}}.muted{{color:#94a3b8}}.auto-note{{color:#94a3b8;font-size:10px;margin-top:2px}}h2{{margin:0 0 3px;font-size:13px;font-weight:700;color:#0f172a}}.topgrid{{display:grid;grid-template-columns:repeat(2,minmax(160px,1fr));gap:6px;margin-bottom:6px}}.stat,.panel{{background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:5px 8px;box-shadow:0 1px 2px #00000008}}.stat span,.mini-stats span{{display:block;color:#64748b;font-size:9px;font-weight:500}}.stat b{{font-size:14px;color:#0f172a}}.panel{{margin-bottom:4px}}.mini-stats{{display:grid;grid-template-columns:repeat(5,minmax(60px,1fr));gap:3px;margin:3px 0 4px}}.mini-stats>div{{background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:3px 5px}}.mini-stats b{{font-size:12px;color:#0f172a}}table{{width:100%;border-collapse:separate;border-spacing:0;font-size:10px}}th,td{{text-align:left;border-bottom:1px solid #e2e8f0;padding:4px 5px;vertical-align:top}}th{{color:#64748b;font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.04em;background:#f8fafc;position:sticky;top:0;z-index:1}}td b{{display:block;font-weight:600}}td a{{font-weight:600}}.num{{text-align:center;font-weight:700;white-space:nowrap}}.refresh{{background:#2563eb;color:#fff;border:0;border-radius:7px;padding:5px 10px;font-weight:700;font-size:11px;cursor:pointer;transition:.15s}}.refresh:hover{{background:#1d4ed8}}details summary{{cursor:pointer;color:#2563eb;font-weight:700;font-size:11px;margin-bottom:3px}}.tablebox{{overflow:auto;border-radius:5px;border:1px solid #e2e8f0;background:#fff}}.unigrid{{display:grid;grid-template-columns:repeat(6,minmax(80px,1fr));gap:3px}}.unigrid>div{{background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:3px 5px}}.unigrid span{{display:block;color:#64748b;font-size:9px;font-weight:500}}.unigrid b{{font-size:12px;color:#0f172a}}@media(max-width:900px){{.topgrid,.mini-stats,.unigrid{{grid-template-columns:1fr 1fr}}table{{min-width:700px}}.wrap{{padding:4px}}}}
</style></head><body><div class="wrap">
<header><div><h1>Дашборд всех проектов</h1><div class="server-meta"><span>🇷🇺 Server1</span><span>📡 {esc(s['ip'])}</span><span>🟢 В работе</span><span>Действует до {esc(s['valid_until'])}</span><span class="muted">Обновлено {esc(s['generated'])}</span></div><div class="auto-note">Автообновление с сервера каждые 2 минуты</div></div><button class="refresh" onclick="fetch('/api/refresh').then(r=>r.json()).then(d=>d.ok?location.reload():alert(d.message)).catch(()=>location.reload())">↻</button></header>
