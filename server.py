...
<style>
*{{box-sizing:border-box}}body{{margin:0;font-family:'Inter','SF Pro',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f0f2f5;color:#111827;font-size:15px;line-height:1.6}}a{{color:#2563eb;text-decoration:none}}a:hover{{text-decoration:underline;color:#1d4ed8}}.wrap{{max-width:1500px;margin:0 auto;padding:18px;text-align:left}}header{{display:inline-flex;gap:4px;margin-bottom:12px;background:#fff;border-radius:12px;padding:10px 16px;box-shadow:0 1px 4px #0000000a;border:1px solid #e2e8f0}}.header-row{{display:flex;align-items:center;gap:10px;flex-wrap:wrap}}h1{{margin:0;font-size:24px;font-weight:700;letter-spacing:-.03em;color:#0f172a}}.muted{{color:#94a3b8}}h2{{margin:0 0 7px;font-size:16px;font-weight:700;color:#0f172a;display:flex;align-items:center;gap:6px}}.stat,.panel{{background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:12px 16px;box-shadow:0 1px 3px #00000006;display:inline-block;vertical-align:top;margin-right:8px;width:auto;min-width:280px;border-left:3px solid #d1d9e6}}.panel{{margin-bottom:8px;border-left-color:#bfd4f0}}.stat span,.mini-stats span{{display:block;color:#64748b;font-size:12px;font-weight:500}}.stat b{{font-size:20px;color:#0f172a}}.mini-stats{{display:grid;grid-template-columns:repeat(5,minmax(90px,1fr));gap:6px;margin:8px 0 10px}}.mini-stats>div{{background:#f8fafc;border:1px solid #e2e8f0;border-radius:7px;padding:7px 10px}}.mini-stats b{{font-size:16px;color:#0f172a}}table{{width:auto;border-collapse:separate;border-spacing:0;font-size:13px}}th,td{{text-align:left;border-bottom:1px solid #e2e8f0;padding:7px 9px;vertical-align:top;white-space:nowrap}}th{{color:#64748b;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.04em;background:#f8fafc;position:sticky;top:0;z-index:1}}td b{{display:block;font-weight:600}}td a{{font-weight:600}}.num{{text-align:center;font-weight:700;white-space:nowrap}}details summary{{cursor:pointer;color:#2563eb;font-weight:700;font-size:13px;margin-bottom:4px}}.tablebox{{display:inline-block;overflow:auto;border-radius:6px;border:1px solid #e2e8f0;background:#fff}}.unigrid{{display:grid;grid-template-columns:repeat(6,minmax(100px,1fr));gap:6px}}.unigrid>div{{background:#f8fafc;border:1px solid #e2e8f0;border-radius:7px;padding:7px 10px}}.unigrid span{{display:block;color:#64748b;font-size:12px;font-weight:500}}.unigrid b{{font-size:16px;color:#0f172a}}@media(max-width:900px){{.topgrid,.mini-stats,.unigrid{{grid-template-columns:1fr 1fr}}table{{min-width:auto}}.wrap{{padding:12px}}}}
</style></head><body><div class="wrap">
<header><div class="header-row"><h1 style="margin:0;font-size:24px;white-space:nowrap">Дашборд всех проектов</h1><span style="background:#f0f4fe;border:1px solid #d1d9e6;border-radius:7px;padding:3px 12px;white-space:nowrap;font-size:12px">💾 <b>{esc(resources['memory']['free'])}</b> из {esc(resources['memory']['total'])}</span><span style="background:#f0f4fe;border:1px solid #d1d9e6;border-radius:7px;padding:3px 12px;white-space:nowrap;font-size:12px">💿 <b>{esc(resources['disk']['free'])}</b> из {esc(resources['disk']['total'])}</span><span style="background:#e1ebfa;border:1px solid #b8cbe8;border-radius:7px;padding:3px 14px;white-space:nowrap;font-size:12px;color:#1e3a6b">🇩🇪 Server1 · 📡 {esc(s['ip'])} · 🟢 В работе · Действует до {esc(s['valid_until'])}</span><span style="background:#f0f4fe;border:1px solid #d1d9e6;border-radius:7px;padding:3px 12px;white-space:nowrap;font-size:12px">Обновлено {esc(s['generated'])}</span></div></header>
<div style="display:inline-block;vertical-align:top;margin-right:8px"><section class="panel" style="display:block;margin-bottom:8px"><details><summary style="cursor:pointer;color:#475569;font-weight:600;font-size:12px;padding:4px 0">Cron всех проектов</summary><div class="tablebox" style="margin-top:6px"><table><thead><tr><th>Профиль</th><th>Задача</th><th>Вкл</th><th>Расписание</th><th>Последний запуск</th><th>Следующий</th><th>Статус</th><th>Ошибка</th></tr></thead><tbody>{cron_html}</tbody></table></div></details></section>
<section class="panel" style="display:block"><h2>Проект Wibes</h2>{metric_cards(blocks['wibes'])}{project_table(blocks['wibes'])}</section></div>
