...
<header><div><h1>Дашборд всех проектов</h1><div class="server-meta"><span>🌐 {esc(s['host'])}</span><span>📡 {esc(s['ip'])}</span><span>🟢 {esc(s['state'])}</span><span>📅 до {esc(s['valid_until'])}</span><span class="muted">обновлено: {esc(s['generated'])}</span></div><div class="auto-note">Автообновление с сервера каждые 2 минуты</div></div><button class="refresh" onclick="location.reload()">↻</button></header>
